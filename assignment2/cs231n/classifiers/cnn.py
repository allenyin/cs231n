...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        # First conv layer
        # Filters are 3x7x7 per depth slice
        self.params['W1'] = np.random.randn(num_filters, C, filter_size, filter_size) * weight_scale 
        self.params['b1'] = np.zeros((num_filters))

        # Hidden affine layer
        # After CNN layer, data volume is FxHxW
        # Pool layer reduces by half -> Fx(H/2)x(W/2)
        # So each affine neuron has Fx(H/2)x(W/2) weights for inputs, total hidden_dim out
        self.params['W2'] = np.random.randn(num_filters*(H//2)*(W//2), hidden_dim) * weight_scale
        self.params['b2'] = np.zeros((hidden_dim))

        # Output affine layer
        # After affine layer, data size is hidden_dim, output to num_classes
    ...
    def loss(self, X, y=None):
        ...
        # now calculate grad
        daffine_relu_out, dW3, db3 = affine_backward(dz, scores_cache)
